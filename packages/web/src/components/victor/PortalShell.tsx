      ...
      <section className="container py-10 md:py-14">
        <div className="mb-8 flex flex-col gap-6 rounded-[2rem] border border-black/5 bg-white/90 p-6 shadow-[0_30px_80px_rgba(65,47,25,0.08)] backdrop-blur md:flex-row md:items-end md:justify-between md:p-8">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link className={linkClass(current === 'dashboard')} to="/dashboard">
              Dashboard
            </Link>
            <Link className={linkClass(current === 'calendar')} to="/calendar">
              Calendar
            </Link>
            <Link className={linkClass(current === 'email')} to="/email">
              Email
            </Link>
            <PortalLogoutButton />
          </div>
