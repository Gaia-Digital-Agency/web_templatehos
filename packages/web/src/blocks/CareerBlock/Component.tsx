...
export const CareerBlock: React.FC<CareerBlockType> = ({
  title,
  description,
  form,
}) => {
  ...
  useEffect(() => {
    Promise.all([
      payloadClient.find('departments', {
        limit: 100,
        sort: 'name',
      }),
      payloadClient.find('team', {
        limit: 100,
        depth: 1,
      }),
    ])
      .then(([deptResult, teamResult]) => {
        setDepartments(deptResult.docs)
        setTeamMembers(teamResult.docs)
      })
      .catch((err) => {
    ...
    <div className="container">
      ...
      <div className="flex flex-col gap-16 mb-20">
        {departments.map((dept: any, i: number) => {
          const deptTeam = teamMembers.filter((member: any) =>
